...
| **Deterministic topology** | 🟡     | Step 1: deterministic build order, working reorder, loud stale-selection errors. Step 2: fingerprint engine (`fingerprint.ts`) + index-renumber-proof resolution | — | Wire stable refs into live pipeline + history propagation (step 3) — see `DETERMINISTIC.md` |
