...
## Operation-panel strategy contract: imperative-handle (pull) → push-based `onChange` draft (2026-07-12)

`src/frontend/ui/operations/strategies/*.tsx` (all 19 registered Strategy panels) and the
`OperationPanel.tsx` shell no longer use `forwardRef`/`useImperativeHandle`. Each panel now reports
its current complete params — or `null` while invalid/incomplete — via a new `onChange: (draft:
PanelDraft | null) => void` prop (`PanelDraft = { params: OperationParams; sketchId?: string }`),
replacing the old `onValidChange` + imperative `submit()` split. A shared
`strategies/shared/useReportDraft.ts` hook does the reporting: panels build the draft inline during
render and the hook pushes it via a JSON-content-keyed effect — no per-panel hand-maintained
dependency arrays. The
shell just holds the latest draft in state and derives Apply's `disabled` state and its params/
sketchId argument from it — no ref, no reaching into the child.

Panels that pass a `sketchId` in the draft (unchanged from what they passed to the old `onConfirm`):
`ExtrudePanel`, `RevolvePanel`, `SweepPanel` (profile sketch), `LoftPanel` (first profile sketch).
All others omit it.

Draft reset on operation switch is handled by `key={operation}` on the rendered Strategy alone: the
remounted panel's mount effect always reports a fresh draft (valid panels a draft, invalid ones
`null`), overwriting any stale one. (An interim `useEffect(() => setDraft(null), [operation])` reset
in the shell was a real bug — React fires a newly-mounted child's effects *before* the parent's own
effects in the same commit, so the parent's reset clobbered the fresh panel's first report; caught
by `CADLayout.test.tsx`'s "should trigger rebuild when status is ready and features exist".)

Verified with `bun run build` (clean), `bun run test` (71 files / 615 tests, all pass — updated
`OperationPanel.test.tsx`'s Phase-4 selector-persistence assertion for the new two-arg `onConfirm(params,
sketchId)` call shape), and `bun run lint` (no new issues in any touched file; pre-existing
`no-explicit-any` errors elsewhere untouched). `grep -rn "useImperativeHandle\|OperationPanelHandle\|
onValidChange" src/` returns nothing.
