...
`OperationPanel.test.tsx` tests; 460/460 tests green; `bun run build` clean.

Then Phase 4 (persistent parametric selectors): `FilletParams`/`ChamferParams`/`ShellParams` gained
an optional `selector?: string`. `modifications.ts`'s new `withSelectorMatches` helper re-evaluates
it against the *live* body on every `applyFillet`/`applyChamfer`/`applyShell` call
(`describeSubShapes` + `selectSubShapes`) and unions the matches with the explicit `edges`/`faces`,
deduped by `refLabel` — decided precedence is **selector ∪ explicit** (additive, never drops a
